...
public class BCAccountDao extends ExtendDaoSupper<BCAccount, BCAccountExample, BCAccountKey> {
	...
	public int batchUpdate(List<BCAccount> records) throws Exception {
		...
		for (BCAccount record : records) {
			int rsize = mapper.updateByPrimaryKeySelective(record);
			if (rsize > 0) {
				size = size + rsize;
			}
		}
	...
	public int batchInsert(List<BCAccount> records) throws Exception {
		...
		try {
			...
			sb.append(
					"INSERT INTO BC_ACCOUNT(ACT_ADDRESS,ACT_NONCE,ACT_BALANCE,ACT_MAX,ACT_ACCEPT_MAX,ACT_ACCEPT_LIMIT,STORAGE_HEX,CODE_HASH_HEX,CODE_HEX,DATA_HEX,ACT_STATUS,CREATE_TIME,UPDATE_TIME) values");
